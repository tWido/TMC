...
int StartRelMoveC(std::vector<string> args){
    ...
    if (opened_device.bays == -1){
        ...
        index += 0x20;
        if (dist_spec) ret = device_calls::StartRelativeMove(dist, index);
        else ret = device_calls::StartSetRelativeMove(index);
        switch (ret){
            case INVALID_DEST:  {printf("  Wrong address given\n"); return ERR_CALL;}
        };
    }
...
int StartAbsMoveC(std::vector<string> args){
    ...
    int ret = 0;
    if (opened_device.bays == -1){
        if (pos_spec) ret = device_calls::StartAbsoluteMove(pos, 0x50, index);
        else ret = device_calls::StartSetAbsoluteMove(0x50, index); // DOSEN'T WORK
        switch (ret){
            case INVALID_CHANNEL: {printf("  Not existing channel given\n"); return ERR_CALL;}
        ...
        index += 0x20;
        if (pos_spec) ret = device_calls::StartAbsoluteMove(pos, 0x50, index);
        else ret = device_calls::StartSetAbsoluteMove(0x50, index);
        switch (ret){
            case INVALID_DEST:  {printf("  Wrong address given\n"); return ERR_CALL;}
            case INVALID_PARAM_1: {printf("  Position given exceeds maximal position\n"); return ERR_CALL;}
        };
...
void PrintStatus(int at){
    ...
    if (opened_device.functions.count(GET_DCSTATUSUPDATE) == 1) printf("  Velocity: %hu\n", opened_device.motor[at].status_velocity);
